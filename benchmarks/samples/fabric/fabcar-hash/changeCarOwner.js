...
class ChangeCarOwnerWorkload extends WorkloadModuleBase {
    ...
    async submitTransaction() {
        try {
            ...
            const newOwner = owners[Math.floor(Math.random() * owners.length)];
            const resUpdate = await cars.findOneAndUpdate({ _id: carNumber }, { $set: { owner: newOwner } }, { returnOriginal: false });
            const newCarHash = crypto.createHash('md5').update(JSON.stringify(resUpdate.value)).digest('hex');
    
            const args = {
                contractId: 'fabcar-hash',
                contractVersion: 'v1',
                contractFunction: 'changeCarOwner',
                contractArguments: [carNumber, newCarHash],
                timeout: 60
            };
